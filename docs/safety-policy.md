...
The prepared resource-demand path establishes environmental feasibility only when all exact
equipment requirements are currently reported available and at least 2 m² of usable floor space is
recorded. Standard push-up has no equipment requirement; that does not certify wrist, elbow,
shoulder, trunk-control, technique, pain tolerance, medical clearance, or same-day readiness. The
prepared two-slot time reservation is not permission to train and contains no exercise dose. Every
eventual session remains blocked by the separate current safety gate and reviewed dose/stop-rule
authority.
...
The prepared Week 1 action may create dated sessions and assign its exact ratified session-safety
policy to the athlete, but neither action is a safety decision. The assignment is included in the
same transaction so the athlete-facing gate has an explicit current policy without hidden CLI
administration. An existing assignment of that same policy is reused; a different current policy
blocks and cannot be silently replaced. Every scheduled occurrence remains blocked behind its own
current pre-session report. Candidate text and the immutable audit explicitly preserve this
